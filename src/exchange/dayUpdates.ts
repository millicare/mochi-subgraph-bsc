...
import { Pair, Bundle, Token, MochiFactory, MochiDayData, PairDayData, TokenDayData } from "../../generated/schema";
import { ONE_BI, ZERO_BD, ZERO_BI, FACTORY_ADDRESS } from "./utils";

export function updateMochiDayData(event: ethereum.Event): MochiDayData {
  let pancake = MochiFactory.load(FACTORY_ADDRESS);
  let timestamp = event.block.timestamp.toI32();
  let dayID = timestamp / 86400;
  let dayStartTimestamp = dayID * 86400;
  let pancakeDayData = MochiDayData.load(dayID.toString());
  if (pancakeDayData === null) {
    pancakeDayData = new MochiDayData(dayID.toString());
  ...
  return pancakeDayData as MochiDayData;
}
